...
load("../../results/additional_sims_lab_block.RData")
...
network_structure_plot <- ggplot(network_structure %>% na.omit()) +
  ...
  geom_point(position=position_jitterdodge(jitter.width=0.1, dodge.width=0.75), colour="#d95d39") +
  coord_trans(y="log1p") +
  ...
  ggtitle("Component-wise structural metrics",
          str_glue("for {num_nets} block model randomizations of the empirical network")) +
...
disease_simulation_plot <- ggplot(disease_simulations %>% filter(metric != "dynamics_unfinished")) +
  ...
  ggtitle("Epidemic Simulation Outcomes",
          str_glue("for {num_sims} simulations on each of {num_nets} randomizations of the empirical network")) +
  theme(axis.title=element_blank(),
        legend.position="bottom",
        legend.title=element_blank(),
        plot.title = element_text(size = rel(1.2), hjust = 0, face="bold", family="Computer Modern",
