...
disease_simulation_plot <- disease_simulations %>%
  ...
  mutate(value = ifelse(metric == "Time to Peak", value, value / size)) %>%
  group_by(metric) %>%
  group_modify(function(data, ...) {
    a <- interp(x=data$size, y=data$connectance, z=zscore(data$value),
                xo=seq(minsize, maxsize, length.out=400),
  ...
  {ggplot(.) +
      ...
      # geom_point(data=disease_simulations %>% filter(metric != "dynamics_unfinished")) +
      facet_wrap(~metric) +
      scale_fill_viridis_c(option="E")}

save.image("additional_sims_ER_size.RData")
# save.image("additional_sims_BA_size.RData")
...
ggsave(disease_simulation_plot, filename="figures/increasing_size_simulations/disease_dynamics.png",  width=9, height=3)
